...
export const getMedia = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const cachedMedia = await redis.get(`media:${id}`);
        let media;
        if(cachedMedia){
            ...
            await redis.setex(`media:${id}`, 86400, JSON.stringify(media));
        }
...
export const getRelatedStories = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    // Find related stories based on tags, author, and publication
    ...
    // Cache for 1 hour
    await redis.setex(cacheKey, 3600, JSON.stringify(relatedStories));
