...
export const getStoryAnalytics = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const cachedAnalytics = await cache.get(cacheKey, [id]);
        ...
        await cache.set(cacheKey, [id], analytics, 300);
...
export const getDashboardAnalytics = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const cachedDashboard = await cache.get(cacheKey, [userId]);
    ...
    await cache.set(cacheKey, [userId], analytics, 600);
...
export const getPublicationAnalytics = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    await cache.set(cacheKey, [id], analytics, 900);
...
export const getEarningsAnalytics = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const cachedEarnings = await cache.get(cacheKey, [userId]);
    ...
    await cache.set(cacheKey, [userId], earnings, 3600);
