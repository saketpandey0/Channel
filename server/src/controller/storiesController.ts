...
import storyValidation from "../validators/storyValidation";
...
export const getStory = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const cachedStory = await cache.get('story', [id, userId]);
        if (cachedStory) {
        ...
        await cache.set('story', [id, userId], story, 600);
...
export const getStories = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const cachedResult = await cache.get('stories', cacheKey);
    if (cachedResult) {
    ...
    await cache.set('stories', cacheKey, result, 300);
...
export const getFeed = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const cachedFeed = await cache.get('user', cacheKey);
        if (cachedFeed) {
        ...
        await cache.set('user', cacheKey, result, 300);
...
export const getTrendingStories = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        await cache.set('stories', cacheKey, result, 600);
...
export const getUserDrafts = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const cachedDrafts = await cache.get('user', [userId, 'drafts']);
        if (cachedDrafts) {
        ...
        await cache.set('user', [userId, 'drafts'], result, 300);
...
export const getUserPublishedStories = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const cachedStories = await cache.get('user', [userId, 'published']);
        if (cachedStories) {
        ...
        await cache.set('user', [userId, 'published'], result, 300);
...
export const getStoriesStats = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const cachedStats = await cache.get('story', [id, 'stats']);
        if (cachedStats) {
        ...
        await cache.set('story', [id, 'stats'], result, 120);
...
export const getStoryVersions = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const cachedVersions = await cache.get('story', [id, 'versions']);
        if (cachedVersions) {
        ...
        await cache.set('story', [id, 'versions'], result, 900);
